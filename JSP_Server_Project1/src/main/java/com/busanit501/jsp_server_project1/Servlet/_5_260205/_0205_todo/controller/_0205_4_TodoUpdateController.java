...
public class _0205_4_TodoUpdateController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        log.info("/todo/update_0205, 수정 폼 임시화면 get으로 요청 처리함. ");

        try {
            //상세조회에서 tno를 이미 불러왔고 tno를 이용해서 기존 데이터를 화면에 불러옴
            Long tno = Long.parseLong(req.getParameter("tno"));
            _0205_TodoDTO todoDTO = todoService.get(tno);

            //화면에 전달
            req.setAttribute("dto", todoDTO);
            req.getRequestDispatcher("/WEB-INF/Servlet/_5_260205/_0205_todo/todoUpdate.jsp").forward(req, resp);
